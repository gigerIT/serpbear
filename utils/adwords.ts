...
const getAdwordsAccessTokenCacheKey = ({
  client_id,
  account_id,
  refresh_token,
}: Pick<AdwordsCredentials, 'client_id' | 'account_id' | 'refresh_token'>) =>
  `adwords_token:${client_id}:${account_id}:${refresh_token}`;

export const clearAdwordsAccessTokenCache = () => {
  memoryCache.clear();
};
 ...
 */
export const getAdwordsCredentials = async (): Promise<
  false | AdwordsCredentials
> => {
  try {
    ...
    try {
      ...
      const client_id = settings.adwords_client_id
        ? cryptr.decrypt(settings.adwords_client_id)
        : '';
      const client_secret = settings.adwords_client_secret
        ? cryptr.decrypt(settings.adwords_client_secret)
        : '';
      const developer_token = settings.adwords_developer_token
        ? cryptr.decrypt(settings.adwords_developer_token)
        : '';
      const account_id = settings.adwords_account_id
        ? cryptr.decrypt(settings.adwords_account_id)
        : '';
      const refresh_token = settings.adwords_refresh_token
        ? cryptr.decrypt(settings.adwords_refresh_token)
        : '';
 ...
 */
export const getAdwordsAccessToken = async (
 ...
 */
export const getAdwordsKeywordIdeas = async (
  credentials: AdwordsCredentials,
  ...
  const cacheKey = getAdwordsAccessTokenCacheKey(credentials);
  const cachedAccessToken: string | false | undefined = memoryCache.get(cacheKey);
  if (cachedAccessToken && !test) {
    accessToken = cachedAccessToken;
  } else {
    accessToken = await getAdwordsAccessToken(credentials);
    memoryCache.delete(cacheKey);
    memoryCache.set(cacheKey, accessToken, { ttl: 3300000 });
  }
  ...
  if (accessToken) {
    ...
    if (seedType === 'searchconsole' && domain) {
      ...
      if (domainSCData && domainSCData.thirtyDays) {
        ...
        const sortedSCKeywords = scKeywords.sort((a, b) =>
          (b.impressions > a.impressions ? 1 : -1),
        );
    ...
    if (seedType === 'tracking' && domain) {
      ...
      const currentKeywords: KeywordType[] = parseKeywords(
        allKeywords.map((e) => e.get({ plain: true })),
      );
    ...
    try {
      ...
      if (
        ['custom', 'searchconsole', 'tracking'].includes(seedType)
      ...
      if (resp.status !== 200) {
        console.log(
          '[ERROR] Google Ads Response :',
          ideaData?.error?.details[0]?.errors[0]?.message,
        );
        // console.log('Response from Ads :', JSON.stringify(ideaData, null, 2));
      }

      if (ideaData?.results) {
        fetchedKeywords = extractAdwordskeywordIdeas(
          ideaData.results as keywordIdeasResponseItem[],
          { country, domain },
        );
      }
 ...
 */
const extractAdwordskeywordIdeas = (
  keywordIdeas: keywordIdeasResponseItem[],
      ...
      if (keywordIdeaMetrics?.avgMonthlySearches) {
        ...
        if (searchVolume > 100) {
          keywords.push({
            ...
            competition,
            competitionIndex:
              competitionIndex !== null ? parseInt(competitionIndex, 10) : 0,
  ...
  return keywords.sort((a: IdeaKeyword, b: IdeaKeyword) =>
    (b.avgMonthlySearches > a.avgMonthlySearches ? 1 : -1),
  );
};
 ...
 */
export const getKeywordsVolume = async (
  keywords: KeywordType[],
): Promise<{ error?: string; volumes: false | Record<number, number> }> => {
  ...
  if (accessToken) {
    ...
    for (const country in keywordRequests) {
      if (
        Object.hasOwn(keywordRequests, country)
        && keywordRequests[country].length > 0
      ) {
        try {
          ...
          if (resp.status !== 200) {
            console.log(
              '[ERROR] Google Ads Volume Request Response :',
              ideaData?.error?.details[0]?.errors[0]?.message,
            );
          ...
          if (ideaData?.results) {
            if (
              Array.isArray(ideaData.results)
              && ideaData.results.length > 0
            ) {
              const volumeDataObj: Map<string, number> = new Map();
              ideaData.results.forEach(
                (item: {
                  keywordMetrics: keywordIdeasMetrics;
                  text: string;
                }) => {
                  const kwVol = item?.keywordMetrics?.avgMonthlySearches;
                  volumeDataObj.set(
                    `${country}:${item.text}`,
                    kwVol ? parseInt(kwVol, 10) : 0,
                  );
                },
              );
          ...
          console.log(
            '[ERROR] Fetching Keyword Volume from Google Ads :',
            error,
          );
        }
 ...
 */
export const updateKeywordsVolumeData = async (
 ...
 */
export const getLocalKeywordIdeas = async (
  domain: string,
): Promise<false | KeywordIdeasDatabase> => {
  try {
    const domainName = domain.replaceAll('-', '.').replaceAll('_', '-');
    const filename = `IDEAS_${domainName}.json`;
    const keywordIdeasRaw = await readFile(
      `${process.cwd()}/data/${filename}`,
      { encoding: 'utf-8' },
    );
    const keywordIdeasData = JSON.parse(
      keywordIdeasRaw,
 ...
 */
export const updateLocalKeywordIdeas = async (
  domain: string,
  data: IdeaDatabaseUpdateData,
): Promise<boolean> => {
  try {
    const domainName = domain.replaceAll('-', '.').replaceAll('_', '-');
