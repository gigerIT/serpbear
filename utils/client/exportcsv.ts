...
const exportCSV = (
  ...
  let csvHeader =
    "ID,Keyword,Position,URL,Country,City,Device,Updated,Added,Tags\r\n";
  let csvBody = "";
  ...
  if (isSCKeywords) {
    ...
    keywords.forEach((keywordData, index) => {
      const { keyword, position, country, device, clicks, impressions, ctr } =
        keywordData as SCKeywordType;
      csvBody += `${index}, ${keyword}, ${
        position === 0 ? "-" : position
    ...
    keywords.forEach((keywordData) => {
      const {
        ...
        added,
        tags,
      } = keywordData as KeywordType;
      csvBody += `${ID}, ${keyword}, ${position === 0 ? "-" : position}, ${
        url || "-"
      }, ${countries[country][0]}, ${
        city || "-"
 ...
 * @returns {void}
 */
export const exportKeywordIdeas = (
  ...
  const csvHeader =
    "Keyword,Volume,Competition,CompetitionScore,Country,Added\r\n";
  let csvBody = "";
