...
import getServerAppURL from "./getServerAppURL";
...
const generateEmail = async (
  ...
  const emailTemplate = await readFile(
    path.join(__dirname, "..", "..", "..", "..", "email", "email.html"),
    { encoding: "utf-8" }
  );
  ...
  keywords.forEach((keyword) => {
    ...
    const deviceIconImg =
      keyword.device === "desktop" ? desktopIcon : mobileIcon;
    ...
    const countryFlag = `<span title="${keyword.country}">${getFlagEmoji(
      keyword.country
    )}</span>`;
    ...
    const posChangeIcon = positionChange
      ? `<span class="pos_change">${positionChangeIcon} ${positionChange}</span>`
      : "";
    keywordsTable += `<tr class="keyword">
                           <td>${countryFlag} ${deviceIcon} ${
      keyword.keyword
    }</td>
                           <td>${keyword.position}${posChangeIcon}</td>
                           <td>${getBestKeywordPosition(keyword.history)}</td>
                           <td>${timeSince(
                             new Date(keyword.lastUpdated).getTime() / 1000
                           )}</td>
                        </tr>`;
  });
  ...
  const updatedEmail = emailTemplate
    .replace(
      "{{logo}}",
      `<img class="logo_img" src="${serpBearLogo}" alt="SerpBear" width="24" height="24" />`
    )
  ...
  const isConsoleIntegrated =
    ...
    (settings.search_console_client_email &&
      settings.search_console_private_key);
  const htmlWithSCStats = isConsoleIntegrated
    ? await generateGoogleConsoleStats(domainName)
    : "";
 ...
 */
const generateGoogleConsoleStats = async (
  domainName: string
): Promise<string> => {
  ...
  const SCStats =
    localSCData && localSCData.stats && Array.isArray(localSCData.stats)
      ? localSCData.stats.reverse().slice(0, 7)
      : [];
  ...
  if (SCStats.length > 0) {
    scData.stats.html = SCStats.reduce(
      (acc, item) => acc + genColumn(item, "date"),
      ""
    );
  }
  if (keywords.length > 0) {
    scData.keywords.html = keywords
      .slice(0, 5)
      .reduce((acc, item) => acc + genColumn(item, "keyword"), "");
  }
  if (pages.length > 0) {
    scData.pages.html = pages
      .slice(0, 5)
      .reduce((acc, item) => acc + genColumn(item, "page"), "");
  }
  scData.stats.clicks = SCStats.reduce((acc, item) => acc + item.clicks, 0);
  scData.stats.impressions = SCStats.reduce(
  ...
  Object.keys(scData).forEach((itemKey) => {
    ...
    const scItemFirstColName =
      itemKey === "stats"
        ? "Date"
        : `${itemKey[0].toUpperCase()}${itemKey.slice(1)}`;
