...
function shouldFallbackToRun(err) {
  return Boolean(
    err &&
      ...
      err.message.includes("Use run() instead")
  );
}
...
function applyStatementWithFallback(statement, method, bindings, context) {
  const fallbackToRun = () => {
    ...
    if (info && typeof info.lastInsertRowid !== "undefined") {
      context.lastID = Number(info.lastInsertRowid);
    }
...
class Database extends EventEmitter {
  constructor(filename, mode, callback) {
    ...
    try {
      const options = {
        readonly:
          hasFlag(flags, OPEN_READONLY) && !hasFlag(flags, OPEN_READWRITE),
      ...
      if (!options.readonly && this.driver.pragma) {
        try {
          ...
          const walMessage =
            walError && typeof walError.message === "string"
              ? walError.message
              : String(walError);
          logger.warn("Failed to enable WAL mode", { walMessage });
        }
      }
  ...
  executeInternal(method, sql, ...args) {
    ...
    try {
      ...
      if (method === "run" || method === "all" || method === "get") {
        result = applyStatementWithFallback(
          statement,
          method,
          preparedBindings,
          context
