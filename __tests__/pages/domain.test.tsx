...
describe("SingleDomain Page", () => {
  ...
  beforeEach(() => {
    useFetchSettingsFunc.mockImplementation(() => ({
      data: { settings: dummySettings },
      isLoading: false,
    }));
    useFetchDomainsFunc.mockImplementation(() => ({
      data: { domains: [dummyDomain] },
      isLoading: false,
    }));
    useFetchKeywordsFunc.mockImplementation(() => ({
      keywordsData: { keywords: dummyKeywords },
      keywordsLoading: false,
    }));
    const fetchPayload = {
      history: dummyKeywords[0].history || [],
      searchResult: dummyKeywords[0].lastResult || [],
    };
    useFetchSingleKeywordFunc.mockImplementation(() => ({
      data: fetchPayload,
      isLoading: false,
    }));
    ...
    useUpdateSettingsFunc.mockImplementation(() => ({
      mutate: () => {},
      isLoading: false,
    }));
  });
  ...
  it("Country Filter should function properly", async () => {
    ...
    expect(
      document.querySelector(".country_filter .select_list")
    ).toBeVisible();
    const firstCountry = document.querySelector(
      ".country_filter .select_list ul li:nth-child(1)"
    );
    if (firstCountry) fireEvent.click(firstCountry);
    const keywordsCount = document.querySelectorAll(".keyword").length;
    expect(keywordsCount).toBe(
      dummyKeywords.filter(
        (keyword) => keyword.country === dummyKeywords[0].country
      ).length
    );
  });
  ...
  it("Tags Filter should Render & Function properly", async () => {
    ...
    expect(
      document.querySelectorAll(".tags_filter .select_list ul li").length
    ).toBe(1);

    const firstTag = document.querySelector(
      ".tags_filter .select_list ul li:nth-child(1)"
    );
  ...
  it("Sort: Position should sort keywords accordingly", async () => {
    ...
    const topPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(1)"
    );
    if (topPosSortOption) fireEvent.click(topPosSortOption);
    const firstKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
    )?.textContent;
    expect(firstKeywordTitle).toBe("compress image");

    // Test Lowest Position Sort
    if (button) fireEvent.click(button);
    const lowestPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(2)"
    );
    if (lowestPosSortOption) fireEvent.click(lowestPosSortOption);
    const secondKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
    )?.textContent;
  ...
  it("Sort: Date Added should sort keywords accordingly", async () => {
    ...
    // Test Top Position Sort
    const topPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(3)"
    );
    if (topPosSortOption) fireEvent.click(topPosSortOption);
    const firstKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
    ...
    if (button) fireEvent.click(button);
    const lowestPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(4)"
    );
    if (lowestPosSortOption) fireEvent.click(lowestPosSortOption);
    const secondKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
  ...
  it("Sort: Alphabetical should sort keywords accordingly", async () => {
    ...
    // Test Top Position Sort
    const topPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(5)"
    );
    if (topPosSortOption) fireEvent.click(topPosSortOption);
    const firstKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
    ...
    if (button) fireEvent.click(button);
    const lowestPosSortOption = document.querySelector(
      "ul.sort_options li:nth-child(6)"
    );
    if (lowestPosSortOption) fireEvent.click(lowestPosSortOption);
    const secondKeywordTitle = document.querySelector(
      ".domKeywords_keywords .keyword:nth-child(1) a"
