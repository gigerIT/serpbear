  ...
  return (
    <div
      ...
      lg:flex lg:justify-between lg:items-center ${
        selected ? ' bg-indigo-50 keyword--selected' : ''
      } ${lastItem ? 'border-b-0' : ''}`}
    >
      <div className=" w-3/4 font-semibold cursor-pointer lg:flex-1 lg:shrink-0 lg:basis-28 lg:w-auto lg:flex lg:items-center">
        <button
          ...
          onClick={(event) => selectKeyword(ID, event)}
        >
        ...
        >
          <span
            ...
            title={countries[country] && countries[country][0]}
          />
      ...
      >
        ...
        {!updating && positionChange > 0 && (
          <i className=" not-italic ml-1 text-xs text-[#5ed7c3]">
            ▲ {positionChange}
          </i>
        )}
        {!updating && positionChange < 0 && (
          <i className=" not-italic ml-1 text-xs text-red-300">
            ▼ {positionChange}
          </i>
        )}
      </div>
      ...
      <div
        title={
          bestPosition && bestPosition.date
            ? new Date(bestPosition.date).toLocaleDateString('en-US', {
                weekday: 'long',
                year: 'numeric',
                month: 'short',
                day: 'numeric',
      ...
      {chartData.labels.length > 0 && (
        <div
          className={`hidden basis-20 grow-0 cursor-pointer lg:block ${
            !tableColumns.includes('History') ? 'lg:hidden' : ''
          }`}
          onClick={() => showKeywordDetails()}
        >
      ...
      <div className="inline-block mt-[4] top-[-5px] relative lg:flex-1 lg:m-0 lg:top-0 max-w-[150px]">
        ...
        <TimeAgo
          title={dayjs(lastUpdated).format('DD-MMM-YYYY, hh:mm:ss A')}
          date={lastUpdated}
        />
      </div>
      ...
      {showSCData && tableColumns.includes('Search Console') && (
        ...
        >
          <span className="min-w-[40px]">
            ...
            <KeywordPosition
              position={
                keywordData?.scData?.position[
                  scDataType as keyof KeywordSCDataChild
                ] || 0
              }
              type="sc"
            />
          ...
          <span className="min-w-[40px]">
            ...
            {keywordData?.scData?.impressions[
              scDataType as keyof KeywordSCDataChild
            ] || 0}
          </span>
          <span className="min-w-[40px]">
            <span className="lg:hidden">Visits: </span>
            {keywordData?.scData?.visits[
              scDataType as keyof KeywordSCDataChild
            ] || 0}
          </span>
      ...
      <div className="absolute right-4 mt-[-10px] top-2 lg:flex-1 lg:basis-5 lg:grow-0 lg:shrink-0 lg:relative lg:right-[-10px]">
        <button
          className={`keyword_dots rounded px-1 text-indigo-300 hover:bg-indigo-50 ${
            showOptions ? 'bg-indigo-50 text-indigo-600 ' : ''
          }`}
      ...
      {lastUpdateError && lastUpdateError.date && showPositionError && (
        ...
        >
          ...
          <TimeAgo
            title={dayjs(lastUpdateError.date).format(
          ...
          <i
            className="absolute top-0 right-0 ml-2 p-2 font-semibold not-italic cursor-pointer"
            onClick={() => setPositionError(false)}
          >
            <Icon type="close" size={16} color="#999" />
          </i>
          <div className=" border-t-[1px] border-red-100 mt-2 pt-1">
            {lastUpdateError.scraper && (
