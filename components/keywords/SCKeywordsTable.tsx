...
import {
  ...
  SCkeywordsByDevice,
  SCsortKeywords,
} from '../../utils/client/SCsortFilter';
...
const SCKeywordsTable = ({
  domain,
  keywords = [],
  isLoading = true,
  isConsoleIntegrated = true,
}: SCKeywordsTableProps) => {
  ...
  const addedkeywords: string[] = keywordsData?.keywords?.map(
      (key: KeywordType) => `${key.keyword}:${key.country}:${key.device}`,
    ) || [];
  ...
  useEffect(() => {
    ...
    const anchorVisible = finalKeywords[device].some(
      (keyword) => keyword.uid === selectionAnchorId,
    );
    if (!anchorVisible) {
      setSelectionAnchorId(null);
    }
  }, [selectionAnchorId, finalKeywords, device]);

  const selectKeyword = (
    keywordID: string,
    event?: React.MouseEvent<HTMLButtonElement>,
  ) => {
    const visibleKeywords = finalKeywords[device];
  ...
  const visibleSelectedCount = finalKeywords[device].filter((keyword) =>
    selectedKeywords.includes(keyword.uid),
  ).length;
  const selectedAllItems = finalKeywords[device].length > 0
    && visibleSelectedCount === finalKeywords[device].length;
  ...
  const Row = ({ data, index, style }: ListChildComponentProps) => {
    ...
    return (
      <SCKeyword
        ...
        keywordData={keyword}
        isTracked={addedkeywords.includes(
          `${keyword.keyword}:${keyword.country}:${keyword.device}`,
        )}
          ...
          <div className="font-semibold text-sm py-4 px-8 text-gray-500 ">
            <ul className="">
              <li className="inline-block mr-4">
                ...
                >
                  <span className=" bg-indigo-100 text-blue-700 px-1 rounded font-black">
                    +
                  </span>{' '}
        ...
        <div className="domkeywordsTable domkeywordsTable--sckeywords styled-scrollbar w-full overflow-auto min-h-[60vh]">
          <div className=" lg:min-w-[800px]">
            ...
            >
              <span className="domKeywords_head_keyword flex-1 basis-20 w-auto ">
                {finalKeywords[device].length > 0 && (
                  <button
                    data-testid="sc-keywords-select-all"
                    ...
                    onClick={() => {
                      if (selectedAllItems) {
                        const visibleKeywordIds = finalKeywords[device].map(
                          (k: SearchAnalyticsItem) => k.uid,
                        );
                        setSelectedKeywords(
              ...
              <span className="domKeywords_head_position flex-1 basis-40 grow-0 text-center">
                Position
              </span>
            ...
            >
              ...
              {isConsoleIntegrated
                ...
                && finalKeywords[device].length === 0 && (
                  <p className=" p-9 pt-[10%] text-center text-gray-500">
                    Could Not fetch Keyword Data for this Domain from Google
                    Search Console.
                  </p>
                )}
              ...
              {!isConsoleIntegrated && (
                <p className=" p-9 pt-[10%] text-center text-gray-500">
                  Google Search Console has not been Integrated yet. Please
                  follow{' '}
                  <a
