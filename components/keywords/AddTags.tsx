...
const AddTags = ({
  ...
  const [tagInput, setTagInput] = useState(() =>
    keywords.length === 1 ? keywords[0].tags.join(", ") : ""
  );
  ...
  const addTag = () => {
    ...
    keywords.forEach((keyword: KeywordType) => {
      tagsPayload[keyword.ID] =
        keywords.length === 1
          ? tagsArray
          : [...new Set([...keyword.tags, ...tagsArray])];
    });
  ...
  return (
    ...
    >
      <div className="relative">
        {inputError && (
          <span className="absolute top-[-24px] text-red-400 text-sm font-semibold">
            {inputError}
          </span>
        )}
        <span
        ...
        >
          <Icon
            type="tags"
            size={16}
            color={showSuggestions ? "#777" : "#aaa"}
          />
          <Icon
            type={showSuggestions ? "caret-up" : "caret-down"}
            size={14}
            color={showSuggestions ? "#666" : "#aaa"}
          />
        </span>
        <input
          ref={inputRef}
