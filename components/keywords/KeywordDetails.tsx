...
const KeywordDetails = ({
  ...
  const { scrapedCount, skippedCount, resultSegments } = useMemo(() => {
    const results = Array.isArray(keywordSearchResult)
      ? keywordSearchResult
      : [];
  ...
  return (
    <div
      className="keywordDetails fixed w-full h-screen top-0 left-0 z-[99999]"
      onClick={closeOnBGClick}
      data-testid="keywordDetails"
    >
      <div className="keywordDetails absolute w-full lg:w-5/12 bg-white customShadow top-0 right-0 h-screen">
        <div className="keywordDetails__header p-6 border-b border-b-slate-200 text-slate-600">
          <div className="flex items-start justify-between gap-3 pr-10">
            <h3 className="text-lg font-bold flex-1 min-w-0 leading-7">
              <span
                    ...
                    >
                      <h4 className="font-semibold text-blue-700">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                        >{`${seg.item.position}. ${seg.item.title}`}</a>
                      </h4>
                      <a
