...
const AddKeywords = ({
  ...
  const { mutate: addMutate, isLoading: isAdding } = useAddKeywords(() =>
    closeModal(false)
  );
  ...
  const setDeviceType = useCallback(
    (input: string) => {
      ...
      if (newKeywordsData.device.includes(input)) {
        updatedDevice = newKeywordsData.device
          .replace(",", "")
          .replace(input, "");
      } else {
        updatedDevice = newKeywordsData.device
          ? `${newKeywordsData.device},${input}`
          : input;
      }
  ...
  const addKeywords = () => {
    ...
    if (nkwrds.keywords) {
      ...
      const currentKeywords = keywords.map(
        (k) =>
          `${k.keyword}-${k.device}-${k.country}${k.city ? `-${k.city}` : ""}`
      );

      const keywordExist = keywordsArray.filter((k) =>
        devices.some((device) =>
          currentKeywords.includes(
            `${k}-${device}-${nkwrds.country}${
              nkwrds.city ? `-${nkwrds.city}` : ""
            }`
          )
        )
      );
      ...
      if (
        ...
        (keywordsArray.length === 1 ||
          currentKeywords.length === keywordExist.length) &&
        keywordExist.length > 0
      ) {
        ...
        <div>
          ...
          <div className=" my-3 flex justify-between text-sm">
            ...
            <ul className="flex text-xs font-semibold text-gray-500">
              <li
                className={`${deviceTabStyle} mr-2 ${
                  newKeywordsData.device.includes("desktop")
              ...
              >
                ...
                <i className="not-italic hidden lg:inline-block">Desktop</i>
                <Icon
                  type="check"
                  classes={"pl-1"}
                  size={12}
              ...
              <li
                className={`${deviceTabStyle} ${
                  newKeywordsData.device.includes("mobile")
                    ? "  bg-indigo-50 text-indigo-700"
                    : ""
                }`}
              ...
              >
                <Icon type="mobile" />{" "}
                <i className="not-italic hidden lg:inline-block">Mobile</i>
                <Icon
                  type="check"
                  classes={"pl-1"}
                  size={12}
          ...
          <div className="relative">
            ...
            />
            <span
              className="absolute text-gray-400 top-3 left-2 cursor-pointer"
              onClick={() => setShowTagSuggestions(!showTagSuggestions)}
            >
              <Icon
                type="tags"
                size={16}
                color={showTagSuggestions ? "#777" : "#aaa"}
              />
              <Icon
                type={showTagSuggestions ? "caret-up" : "caret-down"}
                size={14}
                color={showTagSuggestions ? "#666" : "#aaa"}
              />
            </span>
            {showTagSuggestions && (
              <ul
            ...
            <input
              className={`w-full border rounded border-gray-200 py-2 px-4 pl-8 
                     outline-none focus:border-indigo-300 ${
                       !allowsCity ? " cursor-not-allowed" : ""
                     } `}
              disabled={!allowsCity}
              title={
                !allowsCity
                  ? `Your scraper ${scraperName} doesn't have city level scraping feature.`
                  : ""
              }
              placeholder={`City (Optional)${
                !allowsCity ? `. Not available for ${scraperName}.` : ""
              }`}
