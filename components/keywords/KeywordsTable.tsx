...
const KeywordsTable = (props: KeywordsTableProps) => {
  ...
  const [showKeyDetails, setShowKeyDetails] = useState<KeywordType | null>(
    null,
  );
  ...
  useWindowResize(() => {
    ...
    if (titleColumnRef.current) {
      setMaxTitleColumnWidth(
        (titleColumnRef.current as HTMLElement).clientWidth,
      );
    }
  });
  ...
  const tableColumns = settings?.keywordsColumns || [
    'Best',
    'History',
    'Volume',
    'Search Console',
  ...
  useEffect(() => {
    ...
    const anchorVisible = processedKeywords[device].some(
      (keyword) => keyword.ID === selectionAnchorId,
    );
    if (!anchorVisible) {
      setSelectionAnchorId(null);
    }
  }, [selectionAnchorId, processedKeywords, device]);
  ...
  const selectKeyword = (
    ...
    if (
      ...
      const anchorIndex = visibleKeywords.findIndex(
        (keyword) => keyword.ID === selectionAnchorId,
      );
      const currentIndex = visibleKeywords.findIndex(
      ...
      if (anchorIndex !== -1 && currentIndex !== -1) {
        ...
        const rangeKeywordIds = visibleKeywords
  ...
  const updateColumns = (column: string) => {
    ...
    updateMutate({
      ...defaultSettings,
      ...settings,
      keywordsColumns: newColumns,
    });
  };

  const shouldHideColumn = useCallback(
    (col: string) => {
      return settings?.keywordsColumns
        && !settings?.keywordsColumns.includes(col)
        ? 'lg:hidden'
        : '';
    },
  ...
  const visibleSelectedCount = processedKeywords[device].filter((keyword) =>
        ...
        >
          <div className=" lg:min-w-[800px]">
            ...
            >
                ...
                className={`domKeywords_head_keyword flex-1 basis-[4rem] w-auto lg:flex-1 
                        ${
                          showSCData && tableColumns.includes('Search Console')
                            ? 'lg:basis-20'
                            : 'lg:basis-10'
                        } lg:w-auto lg:flex lg:items-center `}
              >
                {processedKeywords[device].length > 0 && (
                  <button
                    ...
                    onClick={() => {
                      if (selectedAllItems) {
                        const visibleKeywordIds = processedKeywords[device].map(
                          (k: KeywordType) => k.ID,
                        );
                        setSelectedKeywords(
                ...
                <span
                  className={`inline-block lg:flex lg:items-center 
                           ${
                             showSCData
                             && tableColumns.includes('Search Console')
                               ? 'lg:max-w-[235px]'
                               : ''
                           }`}
                >
              ...
              <span
                className={`domKeywords_head_best flex-1 basis-16 grow-0 text-center  ${shouldHideColumn(
                  'Best',
                )}`}
              >
                Best
              ...
              <span
                className={`domKeywords_head_history flex-1 basis-20 grow-0  ${shouldHideColumn(
                  'History',
                )}`}
              >
                History (7d)
              ...
              <span
                className={`domKeywords_head_volume flex-1 basis-24 grow-0 text-center ${shouldHideColumn(
                  'Volume',
                )}`}
              >
                Volume
              ...
              <span className="domKeywords_head_updated flex-1 relative left-3 max-w-[150px]">
                Updated
              </span>
              {showSCData && tableColumns.includes('Search Console') && (
                <div className="domKeywords_head_sc flex-1 min-w-[170px] lg:max-w-[170px] mr-7 text-center">
                  ...
                  <div>
                    ...
                    >
                      <Icon type="google" size={13} />{' '}
                      {scDataObject[scDataType]}
                      <Icon
                        classes="ml-2"
                        type={showScDataTypes ? 'caret-up' : 'caret-down'}
                        size={10}
                      />
                    </div>
                    {showScDataTypes && (
        ...
        >
          <div className="text-sm">
            <p>
              ...
              {selectedKeywords.length > 1 ? 'these' : 'this'} Keyword?
            </p>
