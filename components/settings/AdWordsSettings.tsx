...
const AdWordsSettings = ({
  settings,
  settingsError,
  updateSettings,
  closeSettings,
}: AdWordsSettingsProps) => {
  const {
    adwords_client_id = '',
  ...
  const { mutate: startAdWordsAuth, isLoading: isStartingAuth } = useStartAdwordsAuth(closeSettings);
  ...
  const hasAllCredentials = adwords_client_id
    && adwords_client_secret
    && adwords_refresh_token
    && adwords_developer_token
    && adwords_account_id;

  const updateAndAuthenticate = () => {
    ...
    <div>
      <div>
        <div className=" border-t border-gray-100 pt-4 pb-0">
          ...
          >
            {isStartingAuth && <Icon type="loading" />}
            <Icon type="google" size={14} />{' '}
          ...
          <div className={!cloudProjectIntegrated ? 'opacity-40' : ''}>
            <div className="settings__section__input mb-4 flex justify-between items-center w-full">
              <SecretField
                ...
                onChange={(developer_token: string) =>
            ...
            <div className="settings__section__input mb-4 flex justify-between items-center w-full">
              <button
                ...
                title={
                  hasAllCredentials
                    ? ''
                    : 'Insert All the data in the above fields to Test the Integration'
                }
