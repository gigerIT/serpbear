...
const GITHUB_REPO_URL = "https://github.com/gigerIT/serpbear";
      ...
      return changeLogData.map(
        ({
          ...
          content: body
            .replaceAll(/^(##|###) \[([^\]]+)\]\(([^)]+)\) \(([^)]+)\)/g, "")
            .replaceAll(
              /\(\[(.*?)\]\((https:\/\/github\.com\/(towfiqi|gigerIT)\/serpbear\/commit\/([a-f0-9]{40}))\)\)/g,
              ""
            ),
        })
      );
          ...
          <div className="changelog-body bg-[#f8f9ff] px-6 pt-4 pb-10 overflow-y-auto styled-scrollbar">
            {changeLogs.map(({ version, content, date, major }) => {
              return (
                <div
                  key={version}
                  className={`domKeywords bg-white rounded mb-6 border ${
                    major ? " border-indigo-400" : "border-transparent"
                  }`}
                >
                  <h4 className=" px-5 py-3 border-b border-b-gray-100 flex justify-between text-indigo-700 font-semibold">
                    <a href={`${GITHUB_REPO_URL}/releases/tag/${version}`}>
                      {version}{" "}
                      {major && (
                    ...
                    <span className=" text-sm text-gray-500">
                      <TimeAgo
                        title={dayjs(date).format("DD-MMM-YYYY, hh:mm:ss A")}
                        date={date}
                      />
                    </span>
                  </h4>
