...
import {
  IdeasSortKeywords,
  IdeasfilterKeywords,
} from '../../utils/client/IdeasSortFilter';
...
const IdeasKeywordsTable = ({
  ...
  const [showKeyDetails, setShowKeyDetails] = useState<IdeaKeyword | null>(
    null,
  );
  ...
  useWindowResize(() =>
    setListHeight(window.innerHeight - (isMobile ? 200 : 400)),
  );

  const finalKeywords: IdeaKeyword[] = useMemo(() => {
    const filteredKeywords = IdeasfilterKeywords(
      showFavorites ? favorites : keywords,
      filterParams,
    );
  ...
  const allTags: string[] = useMemo(() => {
    ...
    keywords.forEach((k) => {
      ...
      const keywordFirstTwoWordsReversed = keywordFirstTwoWords
        .split(' ')
        .reverse()
        .join(' ');
    ...
    [...wordTags].forEach((tag) => {
      const foundTags = keywords.filter(
        (kw) =>
          kw.keyword.includes(tag[0])
          || kw.keyword.includes(tag[0].split(' ').reverse().join(' ')),
      );
  ...
  useEffect(() => {
    ...
    const anchorVisible = finalKeywords.some(
      (keyword) => keyword.uid === selectionAnchorId,
    );
    if (!anchorVisible) {
      setSelectionAnchorId(null);
    }
  }, [selectionAnchorId, finalKeywords]);
  ...
  const selectKeyword = (
    ...
    if (
      ...
      const anchorIndex = finalKeywords.findIndex(
        (keyword) => keyword.uid === selectionAnchorId,
      );
      const currentIndex = finalKeywords.findIndex(
        (keyword) => keyword.uid === keywordID,
      );

      if (anchorIndex !== -1 && currentIndex !== -1) {
        const rangeStart = Math.min(anchorIndex, currentIndex);
        const rangeEnd = Math.max(anchorIndex, currentIndex);
        const rangeKeywordIds = finalKeywords
          .slice(rangeStart, rangeEnd + 1)
          .map((keyword) => keyword.uid);

        setSelectedKeywords((prevSelected) => [
          ...new Set([...prevSelected, ...rangeKeywordIds]),
        ]);
        return;
      }
    }
    ...
    setSelectionAnchorId(keywordID);
  };
  ...
  const visibleSelectedCount = finalKeywords.filter((keyword) =>
    selectedKeywords.includes(keyword.uid),
  ).length;
  const selectedAllItems = finalKeywords.length > 0 && visibleSelectedCount === finalKeywords.length;
      ...
      <div className="domKeywords flex flex-col bg-[white] rounded-md text-sm border mb-5">
        {selectedKeywords.length > 0 && (
          <div className="font-semibold text-sm py-4 px-8 text-gray-500 ">
            <div className={`inline-block ${isResearchPage ? ' mr-2' : ''}`}>
            ...
            {isResearchPage && (
              <SelectField
                ...
                updateField={(updated: string[]) =>
                  updated[0] && setAddKeywordDomain(updated[0])
                }
            ...
            >
              <span className=" text-white bg-blue-700 px-2 py-1 rounded font-semibold">
                + Add Keywords
              </span>
            </a>
          </div>
            ...
            >
              <span className="domKeywords_head_keyword flex-1 basis-20 w-auto ">
                {finalKeywords.length > 0 && (
                  <button
                    data-testid="ideas-keywords-select-all"
                    ...
                    onClick={() => {
                      if (selectedAllItems) {
                        const visibleKeywordIds = finalKeywords.map(
                          (k: IdeaKeyword) => k.uid,
                        );
                        setSelectedKeywords(
                          selectedKeywords.filter(
                            (id) => !visibleKeywordIds.includes(id),
                          ),
                        );
                        return;
                      }
                      ...
                      setSelectedKeywords((prevSelected) => [
                        ...new Set([...prevSelected, ...visibleKeywordIds]),
                      ]);
                    }}
                  >
              ...
              <span className="domKeywords_head_vol flex-1 text-center">
                Monthly Search
              </span>
              <span className="domKeywords_head_trend flex-1 text-center">
                Search Trend
              </span>
              <span className="domKeywords_head_competition flex-1 text-center">
                Competition
              </span>
            </div>
            <div
