...
const KeywordIdea = (props: KeywordIdeaProps) => {
  ...
  const chartData = useMemo(() => {
    const chartDataObj: { labels: string[]; series: number[] } = {
      labels: [],
      series: [],
    };
  ...
  return (
    ...
    >
      <div className=" w-3/4 lg:flex-1 lg:basis-20 lg:w-auto font-semibold cursor-pointer">
        <button
          data-testid={`keyword-checkbox-${uid}`}
          ...
          onClick={(event) => selectKeyword(uid, event)}
        >
        ...
        <button
          className={`ml-2 hover:text-yellow-600 hover:opacity-100 ${
            isFavorite ? 'text-yellow-600' : ' opacity-50'
          }`}
      ...
      <div className="keyword_ctr text-center inline-block ml-4 lg:flex mt-4 relative lg:flex-1 lg:m-0 justify-center">
        ...
        >
          <span className=" inline-block p-1 flex-1">
            {competitionIndex}/100
          </span>
          <span className=" inline-block p-1 flex-1 rounded-e text-white">
            {competition}
          </span>
        </div>
      </div>
