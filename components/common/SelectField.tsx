...
const SelectField = (props: SelectFieldProps) => {
  ...
  const filterOptions = (event: React.FormEvent<HTMLInputElement>) => {
    ...
    options.forEach((option: SelectionOption) => {
      if (
        flags
          ? option.label.toLowerCase().startsWith(userVal)
          : option.label.toLowerCase().includes(userVal)
      ) {
  ...
  return (
    <div
      className={`select font-semibold text-gray-500 relative ${
        inline ? 'inline-block' : 'flex'
      } justify-between items-center`}
    >
      {label && (
        <label className="font-semibold inline-block text-sm text-gray-700 capitalize">
          {label}
        </label>
      )}
      <div
        className={`selected flex border ${rounded} p-1.5 px-4 cursor-pointer select-none ${
          fullWidth ? 'w-full' : 'w-[210px]'
        } 
         min-w-[${minWidth}px] ${showOptions ? 'border-indigo-200' : ''}`}
        onClick={() => setShowOptions(!showOptions)}
      >
        <span className={'w-full inline-block truncate mr-2 text-sm'}>
          {selected.length > 0
      ...
      {showOptions && (
        <div
          className={`select_list mt-1 border absolute min-w-[${minWidth}px] top-[30px] right-0 ${
            fullWidth ? 'w-full' : 'w-[210px]'
          }
            ${
              rounded === 'rounded-3xl' ? 'rounded-lg' : rounded
            } max-h-${maxHeight} bg-white z-50 overflow-y-auto styled-scrollbar`}
        >
          {options.length > 20 && (
            <div className="">
              <input
          ...
          <ul>
            {(options.length > 20 && filterdOptions.length > 0 && filterInput
              ? filterdOptions
              : options
            ).map((opt) => {
              ...
              return (
                <li
                  key={opt.value}
                  className={`select-none cursor-pointer px-3 py-2 hover:bg-[#FCFCFF] text-ellipsis overflow-hidden
                        ${
                          itemActive
