...
const AddDomain = ({ closeModal, domains = [] }: AddDomainProps) => {
  ...
  const { mutate: addMutate, isLoading: isAdding } = useAddDomain(() =>
    closeModal()
  );
  ...
  const addDomain = () => {
    ...
    const existingDomains = new Set(
      domains
        .map((d) => normalizeDomainInput(d.domain))
    ...
    insertedURLs.forEach((url) => {
      ...
      const normalizedDomain = normalizeDomainInput(theURL);
      if (normalizedDomain) {
        if (
          !existingDomains.has(normalizedDomain) &&
          !seenDomains.has(normalizedDomain)
        ) {
          domainsTobeAdded.push(normalizedDomain);
    ...
    if (invalidDomains.length > 0) {
      setNewDomainError(
        `Please Insert Valid Website URL. ${
          invalidDomains.length > 1
            ? `Invalid URLs: ${invalidDomains.join(", ")}`
            : ""
        }`
      );
    ...
    >
      <div data-testid="adddomain_modal">
        ...
        <textarea
          ...
          placeholder={
            "Type or Paste URLs here. Insert Each URL in a New line. eg: \nhttps://mysite.com/ \nhttps://anothersite.com/ "
          }
          value={newDomain}
          autoFocus={true}
        ...
        {newDomainError && (
          <div>
            <span className=" ml-2 block float-right text-red-500 text-xs font-semibold">
              {newDomainError}
            </span>
          </div>
        )}
