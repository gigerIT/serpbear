...
const DomainHeader = ({
  ...
  const daysName = (dayKey: string) =>
    dayKey
      ...
      .replace("Days", " Days");
  const buttonStyle =
    "leading-6 inline-block px-2 py-2 text-gray-500 hover:text-gray-700";
  const buttonLabelStyle = "ml-2 text-sm not-italic lg:invisible lg:opacity-0";
  const tabStyle =
    "rounded rounded-b-none cursor-pointer border-[#e9ebff] border-b-0";
    ...
    <div className="domain_keywords_head w-full ">
      <div>
        ...
        <div className="domain_selector bg-white mt-2 lg:hidden">
          <SelectField
            ...
            updateField={(updateSlug: [string]) =>
              updateSlug && updateSlug[0] && router.push(`${updateSlug[0]}`)
            }
            multiple={false}
            rounded={"rounded"}
          />
      ...
      <div className="flex w-full justify-between mt-4 lg:mt-0">
        <ul className=" max-w-[270px] overflow-auto flex items-end text-sm relative top-[2px] lg:max-w-none">
          ...
          <li
            className={`${tabStyle} ${
              router.pathname === "/domain/console/[slug]"
                ? "bg-white border border-b-0 font-semibold"
                : ""
            }`}
          >
          ...
          <li
            className={`${tabStyle} ${
              router.pathname === "/domain/insight/[slug]"
                ? "bg-white border border-b-0 font-semibold"
                : ""
            }`}
          >
          ...
          <li
            className={`${tabStyle} ${
              router.pathname === "/domain/ideas/[slug]"
                ? "bg-white border border-b-0 font-semibold"
                : ""
            }`}
          >
        ...
        <div className={"flex mb-0 lg:mb-1 lg:mt-3"}>
          ...
          {isInsight && (
            <button className={`${buttonStyle} lg:hidden invisible`}>x</button>
          )}
          <div
          ...
          >
            ...
            {!isConsole && !isInsight && !isIdeas && (
              <button
                ...
                aria-pressed="false"
                onClick={() =>
                  refreshMutate({ ids: [], domain: domain.domain })
                }
              >
          ...
          {!isConsole && !isInsight && !isIdeas && (
            <button
              ...
              className={
                "ml-2 inline-block text-blue-700 font-bold text-sm lg:px-4 lg:py-2"
              }
          ...
          {isConsole && (
            <div className="text-xs pl-4 ml-2 border-l border-gray-200 relative">
              ...
              >
                <Icon type="date" size={13} classes="mr-1" />{" "}
                {daysName(scFilter)}
              </span>
              {ShowSCDates && (
          ...
          {isIdeas && (
            <button
              ...
              className={
                "ml-2 text-blue-700 font-bold text-sm flex items-center lg:px-4 lg:py-2"
              }
