...
const DomainItem = ({
  ...
  return (
    <div
      className={`domItem bg-white border rounded w-full text-sm mb-10 hover:border-indigo-200 ${
        selected ? "" : ""
      }`}
    >
        ...
        >
          ...
          >
            ...
            <img
              ...
              src={
                thumb ||
                `https://www.google.com/s2/favicons?domain=${domain.domain}&sz=128`
              }
              alt={domain.domain}
            />
          </div>
          <div className="domain_details flex-1">
            <h3
              className="font-semibold text-base mb-2 max-w-[200px] text-ellipsis overflow-hidden"
              title={domain.domain}
            >
              {domain.domain}
            </h3>
            {keywordsUpdated && (
              <span className=" text-gray-600 text-xs">
                Updated{" "}
                <TimeAgo
                  title={dayjs(keywordsUpdated).format(
                    "DD-MMM-YYYY, hh:mm:ss A"
                  )}
                  date={keywordsUpdated}
                />
              </span>
            )}
          </div>
        ...
        <div className="flex-1 flex flex-col p-4">
          ...
          <div className="dom_stats flex flex-1 font-semibold text-2xl p-4 pt-5 rounded border border-[#E9EBFF] text-center">
            <div className="flex-1 relative">
              <span className="block text-xs lg:text-sm text-gray-500 mb-1">
                Keywords
              </span>
              {keywordCount}
            </div>
