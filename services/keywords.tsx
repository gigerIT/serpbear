...
export const fetchKeywords = async (router: NextRouter, domain: string) => {
  ...
  const res = await fetch(
    `${window.location.origin}/api/keywords?domain=${domain}`,
    { method: 'GET' },
  );
  return res.json();
};
  ...
  const {
    ...
    onSuccess: (data) => {
      ...
      if (
        ...
        const hasRefreshingKeyword = data.keywords.some(
          (x: KeywordType) => x.updating,
        );
...
export function useAddKeywords(onSuccess: Function) {
  ...
  return useMutation(
    async (keywords: KeywordAddPayload[]) => {
      ...
      const res = await fetch(
        `${window.location.origin}/api/keywords`,
        fetchOpts,
      );
...
export function useDeleteKeywords(onSuccess: Function) {
  ...
  return useMutation(
    async (keywordIDs: number[]) => {
      ...
      const res = await fetch(
        `${window.location.origin}/api/keywords?id=${keywordIds}`,
        { method: 'DELETE' },
      );
      if (res.status >= 400 && res.status < 600) {
        throw new Error('Bad response from server');
...
export function useFavKeywords(onSuccess: Function) {
  ...
  return useMutation(
    async ({ keywordID, sticky }: { keywordID: number; sticky: boolean }) => {
      ...
      const res = await fetch(
        `${window.location.origin}/api/keywords?id=${keywordID}`,
        fetchOpts,
      );
      if (res.status >= 400 && res.status < 600) {
        throw new Error('Bad response from server');
    ...
    },
    {
      onSuccess: async (data) => {
        ...
        toast(isSticky ? 'Keywords Made Favorite!' : 'Keywords Unfavorited!', {
          icon: '✔️',
        });
        queryClient.invalidateQueries(['keywords']);
      },
...
export function useRefreshKeywords(
  ...
  return useMutation(
    async ({ ids = [], domain = '' }: { ids?: number[]; domain?: string }) => {
      ...
      const query = ids.length === 0 && domain
          ? `?id=all&domain=${domain}`
          : `?id=${keywordIds}`;
  ...
  return useQuery(
    ...
    async () => {
      try {
        ...
        const res = await fetch(fetchURL, { method: 'GET' }).then((result) =>
          result.json(),
        );
        if (res.status >= 400 && res.status < 600) {
          throw new Error('Bad response from server');
...
export async function fetchSearchResults(
  ...
  const res = await fetch(
    `${window.location.origin}/api/refresh?keyword=${keyword}&country=${country}&device=${device}`,
    { method: 'GET' },
  );
  if (res.status >= 400 && res.status < 600) {
    if (res.status === 401) {
