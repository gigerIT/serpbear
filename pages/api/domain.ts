...
const getDomain = async (
  req: NextApiRequest,
  res: NextApiResponse<DomainGetResponse>
) => {
  ...
  try {
    ...
    if (parsedDomain && parsedDomain.search_console) {
      try {
        ...
        if (scData) {
          scData.client_email = scData.client_email
            ? cryptr.decrypt(scData.client_email)
            : "";
          scData.private_key = scData.private_key
            ? cryptr.decrypt(scData.private_key)
            : "";
    ...
    if (parsedDomain) {
      parsedDomain.scraper_settings = maskDomainScraperSettings(
