...
const DiscoverPage: NextPage = () => {
  ...
  const { scraper_type = "", available_scrapers = [] } = settings;
  const activeScraper = useMemo(
    () =>
      available_scrapers.find(
        (scraper: { label: string; value: string }) =>
          scraper.value === scraper_type
      ),
    [available_scrapers, scraper_type]
  );
  const { data: domainsData } = useFetchDomains(router);
  const scConnected = !!(
    appSettings && appSettings?.settings?.search_console_integrated
  );
  const {
    data: keywordsData,
    isLoading: keywordsLoading,
    isFetching,
  } = useFetchSCKeywords(router, !!domainsData?.domains?.length && scConnected);

  const theDomains: DomainType[] = (domainsData && domainsData.domains) || [];
  const theKeywords: SearchAnalyticsItem[] = useMemo(() => {
    return keywordsData?.data && keywordsData.data[scDateFilter]
      ? keywordsData.data[scDateFilter]
      : [];
  ...
  const theKeywordsGrouped: SearchAnalyticsItem[] = useMemo(() => {
    return [
      ...theKeywordsReduced.map<SearchAnalyticsItem>(
        (o: SearchAnalyticsItem) => {
          const key = `${o.device}-${o.country}-${o.keyword}`;
  ...
  const domainHasScAPI = useMemo(() => {
    const domainSc = activDomain?.search_console
      ? JSON.parse(activDomain.search_console)
      : {};
    ...
    <div className="Domain ">
        ...
        <div className="domain_keywords px-5 pt-10 lg:px-0 lg:pt-8 w-full">
          {activDomain && activDomain.domain ? (
            <DomainHeader
              ...
              exportCsv={() =>
                exportCSV(theKeywordsGrouped, activDomain.domain, scDateFilter)
              }
      ...
      >
        <DomainSettings
          ...
          systemScraperType={activeScraper?.label || scraper_type}
        />
      </CSSTransition>
      <CSSTransition
        in={showSettings}
      ...
      <Footer
        currentVersion={
          appSettings?.settings?.version ? appSettings.settings.version : ""
        }
      />
    </div>
  );
