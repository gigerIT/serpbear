...
const SingleDomain: NextPage = () => {
  ...
  const [keywordSPollInterval, setKeywordSPollInterval] = useState<
    undefined | number
  >(undefined);
  const { data: appSettingsData, isLoading: isAppSettingsLoading } =
    useFetchSettings();
  const { data: domainsData } = useFetchDomains(router);
  const appSettings: SettingsType = appSettingsData?.settings || {};
  const { scraper_type = "", available_scrapers = [] } = appSettings;
  const activeScraper = useMemo(
    () => available_scrapers.find((scraper) => scraper.value === scraper_type),
    [scraper_type, available_scrapers]
  );
    ...
    <div className="Domain ">
      {(!scraper_type || scraper_type === "none") && !isAppSettingsLoading && (
        <div className=" p-3 bg-red-600 text-white text-sm text-center">
          A Scrapper/Proxy has not been set up Yet. Open Settings to set it up
      ...
      <div className="flex w-full max-w-7xl mx-auto">
        ...
        <div className="domain_keywords px-5 pt-10 lg:px-0 lg:pt-8 w-full">
          ...
          <KeywordsTable
            ...
            isConsoleIntegrated={
              !!(appSettings && appSettings.search_console_integrated) ||
              domainHasScAPI
            }
      ...
      <Footer
        currentVersion={appSettings?.version ? appSettings.version : ""}
      />
    </div>
  );
};
