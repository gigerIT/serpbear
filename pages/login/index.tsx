...
const Login: NextPage = () => {
  ...
  const loginuser = async () => {
    ...
    if (!username || !password) {
      if (!username && !password) {
        loginError = {
          type: "empty_username_password",
          msg: "Please Insert Your App Username & Password to login.",
        };
      }
      if (!username && password) {
        loginError = {
          type: "empty_username",
          msg: "Please Insert Your App Username",
        };
      }
      if (!password && username) {
        loginError = {
          type: "empty_password",
          msg: "Please Insert Your App Password",
        };
      }
      setError(loginError);
      setTimeout(() => {
      ...
      try {
        ...
        const res = await fetch(fetchRoute, fetchOpts).then((result) =>
          result.json()
        );
        // console.log(res);
        if (!res.success) {
        ...
        setError({
          type: "unknown",
          msg: "Could not login, The Server is not responsive.",
        });
        setTimeout(() => {
          setError(null);
        }, 3000);
  ...
  const inputStyle =
    "w-full p-2 border border-gray-200 rounded mb-3 focus:outline-none focus:border-blue-200";
        ...
        <div className="w-80 mt-[-300px]">
          ...
          <div className="relative bg-[white] rounded-md text-sm border p-5">
            ...
            <button
              onClick={() => loginuser()}
              className={
                "py-3 px-5 w-full rounded cursor-pointer bg-blue-700 text-white font-semibold text-sm"
              }
            >
              Login
            </button>
            {error && error.msg && (
              <div
                className={
                  "absolute w-full bottom-[-100px] ml-[-20px] rounded text-center p-3 bg-red-100 text-red-600 text-sm font-semibold"
                }
              >
                {error.msg}
              </div>
